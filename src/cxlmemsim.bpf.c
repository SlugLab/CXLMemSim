...
int BPF_KRETPROBE(malloc_return, void *address)
{
	...
	if (address) {
		...
		if (stats) {
			...
			bpf_map_update_elem(&allocs_ringbuf, &address, info,
					    BPF_ANY);
		}
	}
...
int brk_entry(struct pt_regs *ctx)
{
	...
	if (!proc_info) {
		// 如果不存在，创建新的进程信息
		struct proc_info new_proc_info = { .mem_info = {
							   .current_brk = addr,
							   .total_allocated = 0,
							   .total_freed = 0,
						   } };
		bpf_map_update_elem(&process_map, &pid, &new_proc_info,
				    BPF_ANY);
	} else {
		// 如果存在，更新 brk 值
