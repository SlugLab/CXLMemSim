...
ShmCommunicationManager::ShmCommunicationManager(const std::string &name, bool server_mode)
    : shm_name(name), shm_fd(-1), shm_comm(nullptr), is_server(server_mode), client_id(0), request_sem(nullptr),
      response_sem(nullptr) {
    shm_size = sizeof(ShmCommunication);
}

ShmCommunicationManager::~ShmCommunicationManager() { cleanup(); }
...
bool ShmCommunicationManager::create_shared_memory() {
    ...
    // Map shared memory
    shm_comm = static_cast<ShmCommunication *>(mmap(nullptr, shm_size, PROT_READ | PROT_WRITE, MAP_SHARED, shm_fd, 0));
...
bool ShmCommunicationManager::setup_semaphores() {
    ...
    if (is_server) {
        SPDLOG_DEBUG("Creating semaphores: '{}' and '{}'", shm_comm->request_sem_name, shm_comm->response_sem_name);
        ...
        if (request_sem == SEM_FAILED) {
            SPDLOG_ERROR("Failed to create request semaphore '{}': {}", shm_comm->request_sem_name, strerror(errno));
            return false;
        }

        response_sem = sem_open(shm_comm->response_sem_name, O_CREAT, 0666, 0);
        if (response_sem == SEM_FAILED) {
            SPDLOG_ERROR("Failed to create response semaphore '{}': {}", shm_comm->response_sem_name, strerror(errno));
...
bool ShmCommunicationManager::wait_for_request(uint32_t &out_client_id, ShmRequest &request, int timeout_ms) {
    ...
    for (uint32_t cid = 1; cid <= ShmCommunication::MAX_CLIENTS; cid++) {
        if (!shm_comm->clients[cid - 1].connected.load())
            continue;
...
bool ShmCommunicationManager::connect(uint32_t &assigned_client_id) {
    ...
    for (uint32_t i = 0; i < ShmCommunication::MAX_CLIENTS; i++) {
        ...
        if (shm_comm->clients[i].connected.compare_exchange_strong(expected, true)) {
            ...
            snprintf(shm_comm->clients[i].name, sizeof(shm_comm->clients[i].name), "Client_%d", client_id);
...
bool ShmCommunicationManager::send_request(const ShmRequest &request) {
    if (is_server || !shm_comm || client_id == 0)
        return false;
...
bool ShmCommunicationManager::is_connected() const {
    if (is_server) {
        ...
        return shm_comm && client_id > 0 && client_id <= ShmCommunication::MAX_CLIENTS &&
