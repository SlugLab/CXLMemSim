...
MHSLDDevice::MHSLDDevice(uint64_t capacity, uint32_t num_heads, double read_lat, double write_lat, double bandwidth,
                         const LogPConfig &logp_cfg)
    : total_capacity(capacity), num_heads(num_heads), base_read_latency(read_lat), base_write_latency(write_lat),
...
void MHSLDDevice::rebalance_bandwidth() {
    ...
    for (const auto &h : heads) {
        if (h.active)
            total_alloc += h.allocated_capacity;
    }
...
uint64_t MHSLDDevice::allocate_pool(uint32_t head_id, uint64_t size) {
    if (head_id >= MAX_HEADS || !heads[head_id].active)
        return 0;
...
double MHSLDDevice::read_with_coherency(uint32_t head_id, uint64_t addr, uint64_t timestamp) {
    ...
    if (entry->owner_head == head_id &&
        (entry->owner_state == MHSLDCacheState::EXCLUSIVE || entry->owner_state == MHSLDCacheState::MODIFIED ||
...
double MHSLDDevice::write_with_coherency(uint32_t head_id, uint64_t addr, uint64_t timestamp) {
    ...
    if (entry->owner_head == head_id) {
        if (entry->owner_state == MHSLDCacheState::EXCLUSIVE || entry->owner_state == MHSLDCacheState::MODIFIED) {
    ...
    case MHSLDCacheState::OWNED: {
        ...
        if (entry->owner_head != head_id) {
            ...
            if (entry->owner_state == MHSLDCacheState::MODIFIED || entry->owner_state == MHSLDCacheState::OWNED) {
...
double MHSLDDevice::downgrade_owner(uint64_t addr, uint32_t requesting_head, uint64_t timestamp) {
    MHSLDCachelineInfo *entry = get_entry(addr);
    if (!entry || entry->owner_head == UINT32_MAX)
        return 0.0;

    uint32_t owner = entry->owner_head;
    if (owner == requesting_head)
        return 0.0;
...
FabricLink::FabricLink(uint32_t src, uint32_t dst, const FabricLinkConfig &cfg)
    : config_(cfg), src_node_(src), dst_node_(dst), available_credits_(cfg.max_credits) {}
...
double FabricLink::calculate_traversal_latency(uint64_t timestamp, size_t data_size) {
    ...
    double serialization_ns = static_cast<double>(data_size) / (config_.bandwidth_gbps * 1e9) * 1e9;
...
double FabricLink::get_congestion_delay(uint64_t timestamp) const {
    ...
    double utilization = 1.0 - (static_cast<double>(available_credits_.load()) / config_.max_credits);
...
double FabricLink::get_utilization(uint64_t window_ns) const {
    ...
    double credit_util = 1.0 - (static_cast<double>(available_credits_.load()) / config_.max_credits);
    return credit_util;
}
...
RemoteCXLExpander::RemoteCXLExpander(int id, uint32_t remote_node, uint32_t local_node, uint64_t remote_base,
                                     uint64_t remote_capacity, const FabricLinkConfig &link_cfg)
    : CXLMemExpander(25, 25, 100, 150, id, static_cast<int>(remote_capacity / (1024 * 1024))),
      remote_node_id_(remote_node), local_node_id_(local_node), remote_base_addr_(remote_base),
...
double RemoteCXLExpander::calculate_bandwidth(const std::vector<std::tuple<uint64_t, uint64_t>> &elem) {
    ...
    size_t access_count = 0;
    for (const auto &[timestamp, addr] : elem) {
        if (timestamp >= window_start && addr >= remote_base_addr_ && addr < remote_base_addr_ + remote_capacity_) {
            access_count++;
        }
    }
...
int RemoteCXLExpander::insert(uint64_t timestamp, uint64_t tid, uint64_t phys_addr, uint64_t virt_addr, int index) {
    if (index != this->id)
        return 0;
