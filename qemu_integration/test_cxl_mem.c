...
int main(int argc, char *argv[]) {
    ...
    // Map CXL memory
    cxl_mem_base = mmap(NULL, CXL_MEM_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    ...
    if ((uintptr_t)test_addr % CACHE_LINE_SIZE != 0) {
        test_addr = (volatile uint64_t *)(((uintptr_t)test_addr + CACHE_LINE_SIZE - 1) & ~(CACHE_LINE_SIZE - 1));
    }
    ...
    struct thread_data host1_data = {
        .host_id = 1, .shared_addr = test_addr, .iterations = NUM_ITERATIONS, .conflicts_detected = 0};

    struct thread_data host2_data = {
        .host_id = 2, .shared_addr = test_addr, .iterations = NUM_ITERATIONS, .conflicts_detected = 0};
