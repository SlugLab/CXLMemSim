...
typedef struct {
    ...
    union {
        ...
        struct {
            char shm_name[256];
            /* POSIX shared memory name (e.g., "/cxlmemsim") */
            size_t shm_size;
        ...
        struct {
            char dax_path[256];
            /* Path to DAX device (e.g., "/dev/dax0.0") */
            uint64_t base_offset;
            /* Base offset within DAX device */
            size_t region_size;
...
struct cxl_backend_ops {
    ...
    int (*read)(cxl_backend_t *backend, uint64_t addr, void *data, size_t size, uint64_t *latency_ns);

    /* Write to remote/CXL memory */
    int (*write)(cxl_backend_t *backend, uint64_t addr, const void *data, size_t size, uint64_t *latency_ns);
    ...
    int (*atomic_faa)(cxl_backend_t *backend, uint64_t addr, uint64_t value, uint64_t *old_value);

    /* Atomic compare-and-swap */
    int (*atomic_cas)(cxl_backend_t *backend, uint64_t addr, uint64_t expected, uint64_t desired, uint64_t *old_value);
 ...
 */
cxl_backend_t *cxl_backend_create(cxl_backend_type_t type, const cxl_backend_config_t *config);
...
#define CXL_BACKEND_READ(b, addr, data, size, lat) ((b)->ops->read((b), (addr), (data), (size), (lat)))

#define CXL_BACKEND_WRITE(b, addr, data, size, lat) ((b)->ops->write((b), (addr), (data), (size), (lat)))

#define CXL_BACKEND_BULK_READ(b, addr, data, size) ((b)->ops->bulk_read((b), (addr), (data), (size)))

#define CXL_BACKEND_BULK_WRITE(b, addr, data, size) ((b)->ops->bulk_write((b), (addr), (data), (size)))

#define CXL_BACKEND_FENCE(b) ((b)->ops->fence((b)))

#define CXL_BACKEND_FLUSH(b, addr, size) ((b)->ops->flush((b), (addr), (size)))
...
typedef struct {
    ...
    cxl_cacheline_metadata_t metadata;
...
typedef struct {
    ...
    /* Base address of simulated memory */
    uint64_t memory_size;
    /* Size of simulated memory */
    uint64_t num_cachelines;
    /* Number of cachelines (memory_size/64) */
    uint32_t metadata_enabled;
    /* 1 if metadata transfer is enabled */
    uint32_t entry_size;
    /* Size of each entry (64 or 128 bytes) */
    uint8_t padding[64 - 56];
    /* Pad header to 64 bytes */
    cxl_shm_slot_t slots[];
...
/* Size calculation */
#define CXL_SHM_HEADER_SIZE(nslots) (sizeof(cxl_shm_header_t) + (nslots) * sizeof(cxl_shm_slot_t))
